...
def test_app_set_governor(
    ...
    assert state_before[Pond.governor.str_key()] == _addr_to_hex(
        creator_addr
    )
...
#     expected_pool_tokens = int((a_amount * b_amount) ** 0.5 - Pond._scale)
#     assert balance_deltas[addr][pool_asset] == expected_pool_tokens

#     ratio = _get_ratio_from_state(creator_app_client)
#     expected_ratio = int((a_amount * Pond._scale) / b_amount)
...
#     a_amount = 40000
#     b_amount = int(a_amount * Pond._scale / ratio_before)
...
#         swap_amt, a_supply, b_supply, Pond._scale, Pond._fee
#     )
#     assert balances_delta[addr][b_asset] == int(expected_b_tokens)
...
#             Pond.bootstrap,
#             [
#                 (PondErrors.GroupSizeNot2, add_txn(bootstrap(), "atc")),
#                 (
...
#             ],
#         ) + cases(
#             Pond.bootstrap, [("unauthorized", bootstrap())], fake_client
#         )
...
#                 ),
#                 (
#                     PondErrors.AssetPoolIncorrect,
#                     mint(pool_asset=a_asset),
#                 ),
#             ],
#         ) + cases(
#             Pond.mint,
#             [(PondErrors.SenderInvalid, mint())],
#             fake_client,
#         )
...
#                     PondErrors.AssetAIncorrect
#                     if key == "a_xfer"
#                     else PondErrors.AssetBIncorrect,
...
#                 ),
#                 (
#                     PondErrors.SendAmountTooLow,
#                     override_axfer_amount(mint(), key, 1),
#                 ),
#             ]

#         valid_asset_a_xfer = cases(Pond.mint, valid_asset_xfer("a_xfer"))

#         valid_asset_b_xfer = cases(Pond.mint, valid_asset_xfer("b_xfer"))
...
#             Pond.burn,
#             [
#                 (PondErrors.AssetPoolIncorrect, burn(pool_asset=a_asset)),
#                 (
#                     PondErrors.AssetAIncorrect,
...
#                 ),
#                 (PondErrors.AmountLessThanMinimum, burn(burn_amt=0)),
#                 (
#                     PondErrors.AssetPoolIncorrect,
...
#             Pond.swap,
#             [
#                 (PondErrors.AmountLessThanMinimum, swap(swap_amt=0)),
#                 (
#                     PondErrors.SendAmountTooLow,
#                     swap(swap_amt=1),
#                 ),
...
#     app_state = creator_app_client.get_application_state()
#     return app_state[Pond.ratio.str_key()]
...
#     app_state = creator_app_client.get_application_state()
#     return (
#         int(app_state[Pond.pond_token.str_key()]),
#         int(app_state[Pond.asset_a.str_key()]),
#         int(app_state[Pond.asset_b.str_key()]),
#     )


# def _expect_ratio(a_sup, b_sup):
#     return int((a_sup * Pond._scale) / b_sup)
